...
namespace Presentation
{
    public static class MauiProgram
    {
        ...
        private static void RegisterServices(IServiceCollection services)
        {
            ...
            services.AddSingleton<ILifecycleOwner>(provider => MainActivity.Instance);

            var aiChannel = Channel.CreateUnbounded<SKBitmap>();
            services.AddSingleton(aiChannel);
            services.AddSingleton(aiChannel.Reader);
            services.AddSingleton(aiChannel.Writer);
            ...
            services.AddTransient<IDetectObjectsUseCase>(sp => new DetectObjectsUseCase(
                sp.GetRequiredService<IObjectDetectionService>(),
                sp.GetRequiredService<ILogger<DetectObjectsUseCase>>()
            ));
            services.AddTransient<CapturePhotoUseCase>(sp => new CapturePhotoUseCase(
