...
namespace Infrastructure.Services
{
    ...
    public sealed class FrameDispatcherService : IFrameDispatcherService, IAsyncDisposable
    {
        ...
        public async Task StartFrameDispatchAsync(IAsyncEnumerable<SKBitmap> frameSource, CancellationToken ct)
        {
            ...
            // канал для AI: capacity 1, DropOldest -> всегда держим последний кадр
            ...
            // AI worker - один consumer читает кадры и вызывает обработчики (OnFrameForAi)
            _aiWorkerTask = Task.Run(() => AiWorkerAsync(_aiChannel.Reader, token), token);

            // Main dispatch loop reads from source and fans out frames
            _dispatchTask = Task.Run(() => DispatchLoopAsync(frameSource, token), token);

            // небольшая задержка чтобы дать запуститься
                    ...
                    try
                    {
                        ...
                        if (_aiChannel != null && OnFrameForAi != null)
                        {
                            try
                            {
                                ...
                                if (!written)
                                {
                                    // канал переполнен — освобождаем кадр
                                    aiFrame.Dispose();
                                }
                            }
                            catch (Exception ex)
            ...
            try
            {
                while (await reader.WaitToReadAsync(ct))
                {
                    while (reader.TryRead(out var aiFrame))
                    {
                        try
                        {
                            // передаём кадр подписчикам AI - по контракту они должны владеть кадром и Dispose() его
                            try
                            {
                            ...
                            {
                                // Если подписчик выбросил, логируем. Подписчик может не освободить кадр при исключении,
                                // но мы не станем освобождать, чтобы избежать double-dispose — договоритесь в контракте.
                                _logger.LogError(ex, "Error in AI frame handler");
                                // если вы хотите страховаться — можно вызвать aiFrame.Dispose() здесь,
                                // но тогда подписчики не должны снова Dispose().
                            }
                        }
                        finally
                        {
                            // **Важно**: подразумеваем, что OnFrameForAi handler владеет кадром и удаляет его.
                            // Если вы хотите, чтобы dispatcher освобождал кадр после Invoke — поменяйте контракт.
                        }
                    }
                }
            }
        ...
        // Для совместимости можно оставить синхронный Dispose, но он просто вызывает асинхронный
        public void Dispose()
        {
